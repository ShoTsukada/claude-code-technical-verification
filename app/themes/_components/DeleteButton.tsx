...
export default function DeleteButton({
  ...
  const sizeClass =
    size === "xs"
      ? "h-7 px-3 text-sm min-w-[72px]"
      : "h-9 px-4 text-sm min-w-[80px]";

  const handleConfirm = async () => {
    setLoading(true);
    try {
      await deleteEntity("research_themes", id);
      showToast("success", `「${title}」を削除しました`);
      setOpen(false);
      if (redirectTo) {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        isOpen={open}
        title="研究テーマを削除しますか？"
        message={`「${title}」を削除します。関連する研究機関・投資家との関連付けもすべて削除されます。この操作は元に戻せません。`}
        confirmLabel="削除する"
